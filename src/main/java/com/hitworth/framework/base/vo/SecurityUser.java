 ...
 * <p>
 * Developers may use this class directly, subclass it, or write their own {@link UserDetails}
 * implementation from scratch.
 * <p>
 * {@code equals} and {@code hashcode} implementations are based on the {@code username} property
 * only, as the intention is that lookups of the same user principal object (in a user registry, for
 * example) will match where the objects represent the same user, not just when all the properties
 * (authorities, password for example) are the same.
 * <p>
 * Note that this implementation is not immutable. It implements the {@code CredentialsContainer}
 * interface, in order to allow the password to be erased after authentication. This may cause
 * side-effects if you are storing instances in-memory and reusing them. If so, make sure you return
 * a copy from your {@code UserDetailsService} each time it is invoked.
 * 
 * @author Ben Alex
 * @author Luke Taylor
 */
public class SecurityUser implements UserDetails, CredentialsContainer {
  ...
  public SecurityUser(String username, String password, boolean enabled, boolean accountNonExpired,
    ...
    this(-1,null, null, null, null,username, password, enabled, accountNonExpired, credentialsNonExpired,
   ...
   *         or as an element in the <code>GrantedAuthority</code> collection
   */
  public SecurityUser(int id,String type, String email, String contactPhone,String displayName, String username, String password,
